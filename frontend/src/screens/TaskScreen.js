...
const TaskScreen = () => {
  ...
  }, [dispatch, userInfo, keyword, pageNumber, navigate, canManageTasks, refreshTasks]);
    ...
    {tasks?.length > 0 ? (
      <>
        ...
        >
          ...
          <tbody>
            {tasks.map((task) => {
              const assignedUser = users.find((u) => u.id === task.username) || {};
        ...
        {pages > 1 && (
          <div className="d-flex justify-content-center mt-4">
            <Paginate page={page} pages={pages} keyword={keyword} basePath="/tasks" />
          </div>
        )}
